...
function setup() {
  ...
  confetti1[i] = new Confetti(random(width)-3, random(height)+3);
  ...
  for (let i = 0; i < 65; i++) {
    confetti[i] = new Confetti(random(width), random(height * -.2, height * -.1), random(-90, 90));
  }
...
function draw() {
  ...
  let vol_mouth = mic.getLevel()*-.3;
  let mob = color(114, 122, 135);
